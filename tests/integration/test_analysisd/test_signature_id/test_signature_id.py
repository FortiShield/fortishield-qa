...
references:
    - https://fortishield.github.io/documentation/current/user-manual/ruleset/ruleset-xml-syntax/rules.html#if-sid
'''
import os
import pytest

from fortishield_testing import LOG_FILE_PATH, T_5, T_10
...
from fortishield_testing.modules.analysisd import event_monitor as ev
...
def test_valid_signature_id(configuration, metadata, set_fortishield_configuration, truncate_monitored_files,
                            prepare_custom_rules_file, restart_fortishield_function):
    '''
    ...
    parameters:
        ...
        - restart_fortishield_function:
            type: fixture
            brief: Restart fortishield at the start of the module to apply configuration.
    ...
    with pytest.raises(TimeoutError):
        ev.check_if_sid_not_found(fortishield_log_monitor)
    # Check that no log appears for rules if_sid field being empty string
    with pytest.raises(TimeoutError):
        ev.check_empty_if_sid(fortishield_log_monitor)
    # Check that no log appears for rules if_sid field being invalid
    with pytest.raises(TimeoutError):
        ev.check_invalid_if_sid(fortishield_log_monitor, is_empty=False)


@pytest.mark.tier(level=1)
@pytest.mark.parametrize('configuration, metadata', zip(t2_configurations, t2_configuration_metadata), ids=t2_case_ids)
def test_invalid_signature_id(configuration, metadata, set_fortishield_configuration, truncate_monitored_files,
                              prepare_custom_rules_file, restart_fortishield_function):
    '''
    ...
    ev.check_invalid_if_sid(fortishield_log_monitor, metadata['is_empty'])


# Tests
@pytest.mark.tier(level=1)
@pytest.mark.parametrize('configuration, metadata', zip(t3_configurations, t3_configuration_metadata), ids=t3_case_ids)
def test_null_signature_id(configuration, metadata, set_fortishield_configuration, truncate_monitored_files,
                           prepare_custom_rules_file, restart_fortishield_function):
    '''
