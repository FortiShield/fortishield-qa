...
def get_local_internal_options(request):
    ...
    if sys.platform == 'win32':
        conf.add_fortishield_local_internal_options({'\n windows.debug': '2'})
    else:
        conf.add_fortishield_local_internal_options({'\n logcollector.debug': '2'})
    conf.add_fortishield_local_internal_options({'logcollector.state_interval': request.param})
...
def test_options_state_interval(get_local_internal_options, file_monitoring):
    ...
    description: Check if the 'fortishield-logcollector' daemon updates the statistic file 'fortishield-logcollector.state'
                 ...
                 beetwen the allowed limits, if not, it will verify that the Fortishield is stopped (on Windows systems)
                 or an error event is generated (on Linux systems). Finally, if the interval value is valid, it
                 will verify that the 'fortishield-logcollector.state' file is updated at the specified intervals by
