...
brief: The 'fortishield-logcollector' daemon monitors configured files and commands for new log messages. When Fortishield is
       configured incorrectly then a configuration error is displayed in the Fortishield's log, and Fortishield does not start (if
...
references:
    ...
    - https://fortishield.github.io/documentation/current/user-manual/reference/ossec-conf/localfile.html#location
    - https://fortishield.github.io/documentation/current/user-manual/reference/ossec-conf/localfile.html#log-format
...
from fortishield_testing.tools import LOGCOLLECTOR_DAEMON, LOG_FILE_PATH
from fortishield_testing.tools.configuration import load_fortishield_configurations
from fortishield_testing.tools.services import check_daemon_status
from fortishield_testing.tools.utils import lower_case_key_dictionary_array
from fortishield_testing.tools.monitoring import FileMonitor
from fortishield_testing.fim import callback_configuration_error
from fortishield_testing.logcollector import LOG_COLLECTOR_GLOBAL_TIMEOUT, callback_missing_element_error
...
parameters = [
    {'LOCATION': os.path.join(temp_dir, 'fortishield-testing', files[0]), 'LOG_FORMAT': None},
...
configurations_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data', 'invalid_fortishield_conf.yaml')
...
def test_invalid_fortishield_conf(get_files_list, create_file_structure_module, get_configuration, remove_empty_options,
    ...
    description: Check if the expected message is present in the ossec.log when an invalid <localfile> configuration is
                 set and if Fortishield refuses to restart.
    ...
    fortishield_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback_missing_element_error,
                            error_message='Did not receive the expected "ERROR: ...: Missing ... element.')

    fortishield_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback_configuration_error,
