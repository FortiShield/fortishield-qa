...
from fortishield_testing.tools.services import search_process, control_service
...
configurations_path = os.path.join(test_data_path, 'fortishield_macos_format_basic.yaml')
...
def test_independent_log_process(get_configuration, configure_environment, file_monitoring,
    ...
    '''
    description: Check if the independent execution of log processes (external to Fortishield) is not altered when
                 the Fortishield agent is started or stopped. For this purpose, the test will configure a 'localfile'
                 ...
                 logs. Then, the test will stop the Fortishield agent, launch a new log process and start it again.
    ...
    parameters:
        ...
        - restart_required_logcollector_function:
            type: fixture
            brief: Restart the Fortishield agent.
        - file_monitoring:
            type: fixture
    ...
    assertions:
        ...
        - Verify that the Fortishield agent does not kill independent log processes when it is started.
        - Verify that the Fortishield agent does not kill independent log processes when it is stopped.

    input_description: A configuration template (test_macos_log_process) is contained in an external YAML
                       file (fortishield_macos_format_basic.yaml). That template is combined with a test case
    ...
    control_service('start', 'fortishield-logcollector')

    assert any(x['pid'] == independent_log_pid for x in search_process('log')), 'The independent log process is dead ' \
                                                                                'after starting Fortishield agent '
    control_service('stop', 'fortishield-logcollector')

    assert any(x['pid'] == independent_log_pid for x in search_process('log')), 'The independent log process is dead ' \
                                                                                'after stopping Fortishield agent '
...
def test_macos_log_process_stop(get_configuration, configure_environment, file_monitoring,
                                restart_required_logcollector_function):
    '''
    description: Check if the 'fortishield-logcollector' daemon stops the 'log' and 'script' process when the Fortishield agent
                 ...
                 logcollector process is finished or the Fortishield agent is stopped, those processes must stop.
                 ...
                 'script' processes are running, stop the 'fortishield-logcollector' daemon, verify that the 'log' and
                 'script' processes are stopped, and start it again. Finally, the test will repeat the previous
                 steps, but stopping and starting the Fortishield agent instead of the 'fortishield-logcollector' daemon.
    ...
    assertions:
        - Verify that the logcollector starts monitoring the macOS ULS log messages.
        - Verify that the 'log' and 'script' processes are finished when the 'fortishield-logcollector' daemon is stopped.
        - Verify that the 'log' and 'script' processes are finished when the fortishield agent is stopped.
    ...
    control_service('stop', daemon='fortishield-logcollector')
    check_process_status(process_to_stop, running=False, stage='after stop logcollector')
    control_service('start', daemon='fortishield-logcollector')
...
def test_macos_log_process_stop_suddenly_warning(get_configuration, configure_environment, file_monitoring,
                                                 restart_required_logcollector_function):
    '''
    description: Check if the 'fortishield-logcollector' daemon generates an error event when the 'log stream' process
    ...
    for killed_process in process_to_kill:
        ...
        control_service('restart', daemon='fortishield-logcollector')
