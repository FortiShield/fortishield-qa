...
configurations_path = join(test_data_path, 'fortishield_macos_file_status_predicate.yaml')
...
def test_macos_file_status_predicate(restart_logcollector_required_daemons_package, truncate_log_file,
    ...
    """
    description: Check if the 'fortishield-logcollector' does not update the 'file_status.json' file from logging
    ...
    input_description: A configuration template (test_macos_file_status_predicate) is contained in an external
                       YAML file (fortishield_macos_file_status_predicate.yaml). That template is combined with
