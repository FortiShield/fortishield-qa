...
brief: The 'fortishield-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'log_format' tag and the Fortishield API returns the same values for the configured
...
default_log_format_configuration = 'fortishield_basic_configuration.yaml'
multiple_logcollector_configuration = 'fortishield_duplicated_macos_configuration.yaml'
no_location_defined_configuration = 'fortishield_no_defined_location_macos_configuration.yaml'
...
macos_tcases = [{'LOCATION': 'macos', 'LOG_FORMAT': 'macos', 'VALID_VALUE': True},
                ...
                {'LOCATION1': 'macos', 'LOG_FORMAT1': 'macos', 'LOCATION2': 'macos', 'LOG_FORMAT2': 'macos',
                 'VALID_VALUE': True, 'CONFIGURATION': 'fortishield_duplicated_macos_configuration.yaml'},
                {'LOG_FORMAT': 'macos', 'VALID_VALUE': True,
                 'CONFIGURATION': 'fortishield_no_defined_location_macos_configuration.yaml'}
                ]
...
configurations = load_fortishield_configurations(configurations_path_default, __name__,
...
configurations += load_fortishield_configurations(configurations_path_multiple_logcollector, __name__,
...
configurations += load_fortishield_configurations(configurations_path_no_location, __name__,
...
def check_log_format_valid(cfg):
    """Check if Fortishield run correctly with the specified log formats.
...
def check_log_format_invalid(cfg):
    ...
    """Check if Fortishield fails because a invalid frequency configuration value.
...
def check_log_file_duplicated():
    """Check if Fortishield shows a warning message when the configuration is duplicated."""
    fortishield_log_monitor = FileMonitor(LOG_FILE_PATH)
...
def test_log_format(configure_local_internal_options_module, get_configuration,
    ...
    '''
    description: Check if the 'fortishield-logcollector' daemon detects invalid configurations for the 'log_format' tag.
                 ...
                 Finally, the test will verify that the Fortishield API returns the same values for the 'localfile' section
    ...
    input_description: A configuration templates (test_basic_configuration_log_format) are contained in externals
                       YAML files (fortishield_basic_configuration.yaml, fortishield_duplicated_macos_configuration.yaml, and
                       fortishield_no_defined_location_macos_configuration.yaml). Those templates are combined with
