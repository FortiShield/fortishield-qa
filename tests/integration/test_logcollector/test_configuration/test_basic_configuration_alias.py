...
brief: The 'fortishield-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       specified in the 'alias' tag when monitoring a command, and the Fortishield API returns the same
...
def test_configuration_alias(configure_local_internal_options_module,
    ...
    '''
    description: Check if the 'fortishield-logcollector' daemon changes a command name in the log messages by
    ...
    input_description: A configuration template (test_basic_configuration_alias) is contained in an external YAML file
                       (fortishield_basic_configuration.yaml). That template is combined with two test cases defined
