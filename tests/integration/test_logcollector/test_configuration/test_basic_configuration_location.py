...
brief: The 'fortishield-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'location' tag is set in the configuration, and the Fortishield API returns the same values for
...
from fortishield_testing.logcollector import WINDOWS_CHANNEL_LIST, callback_eventchannel_bad_format, \
...
def test_configuration_location(get_configuration, configure_environment, restart_logcollector):
    '''
    description: Check if the 'fortishield-logcollector' daemon starts properly when the 'location' tag is used.
                 ...
                 Fortishield component is started by checking its process, and the Fortishield API returns the same
    ...
    assertions:
        - Verify that the Fortishield component (agent or manager) can start when the 'location' tag is used.
    ...
    if fortishield.github.ioponent == 'fortishield-manager':
        ...
        if sys.platform == 'win32':
            assert check_if_process_is_running('fortishield-agent.exe') == True
            ...
            check_if_process_is_running('fortishield-logcollector')
