...
brief: The 'fortishield-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'out_format' tag and the Fortishield API returns the same values for the configured
...
references:
    ...
    - https://fortishield.github.io/documentation/current/user-manual/reference/ossec-conf/localfile.html#out-format
...
def check_configuration_out_format_valid(cfg):
    """Check if the Fortishield run correctly with the specified out format field.
...
def check_configuration_out_format_invalid(cfg):
    """Check if the Fortishield fails because an invalid out format configuration value.
...
def test_configuration_out_format(get_configuration, configure_environment, configure_local_internal_options_module,
    ...
    '''
    description: Check if the 'fortishield-logcollector' daemon detects invalid settings for the 'out_format' tag.
