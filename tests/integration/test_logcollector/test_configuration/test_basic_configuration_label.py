...
brief: The 'fortishield-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'label' tag is set in the configuration, and the Fortishield API returns the same values for
...
references:
    ...
    - https://fortishield.github.io/documentation/current/user-manual/reference/ossec-conf/localfile.html#label
...
def test_configuration_label(get_configuration, configure_environment, restart_logcollector):
    '''
    description: Check if the 'fortishield-logcollector' daemon can monitor log files configured to use labels.
                 ...
                 Finally, the test will verify that the Fortishield API returns the same values for
