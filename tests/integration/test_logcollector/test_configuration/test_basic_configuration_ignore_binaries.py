...
brief: The 'fortishield-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'ignore_binaries' tag and the Fortishield API returns the same values for the configured
...
references:
    ...
    - https://fortishield.github.io/documentation/current/user-manual/reference/ossec-conf/localfile.html#ignore_binaries
...
from fortishield_testing.tools import get_service, LOG_FILE_PATH
...
from fortishield_testing.modules.logcollector import LOG_COLLECTOR_PREFIX, WINDOWS_AGENT_PREFIX
from fortishield_testing.modules.logcollector.event_monitor import check_win_wildcard_pattern_no_match
...
def check_ignore_binaries_valid(cfg):
    """Check if the Fortishield runs correctly with the specified ignore_binaries field value.
...
def check_ignore_binaries_invalid(cfg):
    """Check if the Fortishield fails using a invalid ignore_binaries configuration value.
...
def test_ignore_binaries(get_configuration, configure_environment):
    '''
    description: Check if the 'fortishield-logcollector' daemon detects invalid configurations for the 'ignore_binaries' tag.
                 ...
                 will verify that the Fortishield API returns the same values for the 'localfile' section that
                 the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_ignore_binaries) is contained in
                       an external YAML file (fortishield_basic_configuration.yaml). That template is combined
    ...
    if cfg['valid_value']:
        ...
        if cfg['ignore_binaries'] in problematic_values:
            pytest.xfail("Logcolector accepts invalid values. Issue: https://github.com/fortishield/fortishield/issues/8158")
        else:
            if sys.platform == 'win32':
