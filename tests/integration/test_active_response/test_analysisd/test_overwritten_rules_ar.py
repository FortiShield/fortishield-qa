...
references:
    - https://fortishield.github.io/documentation/current/user-manual/capabilities/active-response/#active-response
...
from fortishield_testing.processes import check_if_daemons_are_running, run_local_command_printing_output
...
from fortishield_testing.tools import CUSTOM_RULES_PATH, LOCAL_RULES_PATH, ACTIVE_RESPONSE_BINARY_PATH
...
fortishield_ar_script = os.path.join(ACTIVE_RESPONSE_BINARY_PATH, 'custom-ar.sh')
output_custom_ar_script = '/tmp/file-ar.txt'
source_path = [custom_rule, local_rules, custom_ar_script]
destination_path = [f"{CUSTOM_RULES_PATH}/0270-web_appsec_rules_edit.xml", LOCAL_RULES_PATH, fortishield_ar_script]
...
def test_overwritten_rules_ar(configuration, metadata, source_path, destination_path, new_file_path,
                              create_file, set_fortishield_configuration, copy_file, truncate_monitored_files,
                              restart_fortishield_function, clean_AR_file):
    '''
    ...
    parameters:
        ...
        - create_file:
            type: fixture
            brief: Create a file to be monitored by Fortishield.
        - set_fortishield_configuration:
            type: fixture
            brief:  Set Fortishield custom configuration.
        - copy_file:
            type: fixture
            brief:  Copy file from source to destination.
        - truncate_monitored_files:
            type: fixture
            brief:  Truncate fortishield log files.
    ...
    '''
    # Set all permissions for fortishield to be able to use the custom active response
    os.chmod(fortishield_ar_script, 0o777)
    ...
    # Check that fortishield-analysisd is running and has not crashed
    assert check_if_daemons_are_running(['fortishield-analysisd'])[0], 'fortishield-analysisd daemon is not running. ' \
