...
references:
    ...
    - https://fortishield.github.io/documentation/current/user-manual/api/configuration.html
...
from fortishield_testing.tools.configuration import check_apply_test, load_fortishield_configurations
...
daemons_handler_configuration = {'daemons': ['fortishield-apid']}
file_to_monitor = API_LOG_FILE_PATH
test_data_path = join(dirname(realpath(__file__)), 'data')
configurations_path = join(test_data_path, 'fortishield_max_upload_size.yaml')
...
def restart_required_api_fortishield():
    """Restart API-required services and stop them all at the end of the test."""
    required_api_daemons = ['fortishield-modulesd', 'fortishield-analysisd', 'fortishield-execd', 'fortishield-db', 'fortishield-remoted']
...
def test_max_upload_size(tags_to_apply, get_configuration, configure_api_environment, restart_required_api_fortishield,
    ...
    parameters:
        ...
        - restart_required_api_fortishield:
            type: fixture
    ...
    input_description: A test case is (test_upload_size) contained in an external YAML file
                       (fortishield_max_upload_size.yaml) which includes API configuration
