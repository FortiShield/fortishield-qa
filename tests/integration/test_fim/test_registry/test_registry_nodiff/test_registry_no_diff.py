...
references:
    ...
    - https://documentation.fortishield.github.io/current/user-manual/reference/ossec-conf/syscheck.html#registry-nodiff
...
from fortishield_testing.fim import LOG_FILE_PATH, registry_value_cud, KEY_WOW64_32KEY, KEY_WOW64_64KEY, generate_params
...
def test_no_diff_str(key, subkey, arch, value_name, truncated, tags_to_apply,
                     ...
                     wait_for_fim_start):
    '''
    description: Check if the 'fortishield-syscheckd' daemon truncates the value changes in the generated events using
    ...
    input_description: A test case (no_diff_str) is contained in an external YAML file (fortishield_conf.yaml)
                       which includes configuration settings for the 'fortishield-syscheckd' daemon. That is
...
def test_no_diff_regex(key, subkey, arch, value_name, truncated, tags_to_apply,
    ...
    input_description: A test case (no_diff_regex) is contained in an external YAML file (fortishield_conf.yaml)
                       which includes configuration settings for the 'fortishield-syscheckd' daemon. That is
                       combined with the testing registry keys to be monitored defined in the module.
