...
brief: Active responses perform various countermeasures to address active
       ...
       correctly to the Fortishield agent by `fortishield-remoted` daemon.
...
from fortishield_testing import UDP, TCP, TCP_UDP
from fortishield_testing.tools.configuration import load_fortishield_configurations
from fortishield_testing.tools.sockets import send_active_response_message
...
configurations_path = os.path.join(test_data_path, 'fortishield_test_active_response.yaml')
...
def test_active_response_ar_sending(get_configuration, configure_environment, restart_remoted):
    ...
    assertions:
        - Verify that the 'fortishield-execd' daemon sends the active response to the 'fortishield-remoted' daemon.
        - Verify that the 'fortishield-remoted' daemon receives the active response from the 'fortishield-execd' daemon.
        - Verify that the Fortishield agent receives an active response message.
    
    input_description: A configuration template (test_active_response_send_ar) is contained in an external YAML
                       file (fortishield_test_active_response.yaml). That template is combined with different
                       test cases defined in the module. Those include configuration settings for
                       the 'fortishield-remoted' daemon.
    ...
    for protocol in protocol_array:
        ...
        try:
            ...
            fortishield_log_monitor.start(timeout=10, callback=log_callback,
