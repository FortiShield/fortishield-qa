...
def get_alert_indexer_api(query, credentials, ip_address, index='fortishield-alerts-4.x-*'):
    """Get an alert from the fortishield-indexer API

      Make a request to the fortishield-indexer API to get the last indexed alert that matches the values passed in
      must_match.

      Args:
          ip_address (str): fortishield-indexer IP address.
...
def delete_index_api(credentials, ip_address, index='fortishield-alerts-4.x-*'):
    """Delete indices from fortishield-indexer using its API.

      Make a request to the fortishield-indexer API to delete indices that match a given name.
