...
from fortishield_testing.qa_ctl.provisioning.ansible import read_ansible_instance, remove_known_host
from fortishield_testing.qa_ctl.provisioning.ansible.ansible_inventory import AnsibleInventory
from fortishield_testing.qa_ctl.provisioning.fortishield_deployment.fortishield_local_package import FortishieldLocalPackage
from fortishield_testing.qa_ctl.provisioning.fortishield_deployment.fortishield_s3_package import FortishieldS3Package
from fortishield_testing.qa_ctl.provisioning.fortishield_deployment.fortishield_sources import FortishieldSources
from fortishield_testing.qa_ctl.provisioning.fortishield_deployment.agent_deployment import AgentDeployment
from fortishield_testing.qa_ctl.provisioning.fortishield_deployment.manager_deployment import ManagerDeployment
...
from fortishield_testing.qa_ctl.provisioning.qa_framework.qa_framework import QAFramework
...
from fortishield_testing.qa_ctl.provisioning.local_actions import qa_ctl_docker_run


class QAProvisioning():
    """Class to control different options and instances to provisioning with Fortishield and QA Framework.
    ...
    Attributes:
        ...
        fortishield_installation_paths (dict): Dict indicating the Fortishield installation paths for every host.
        qa_ctl_configuration (QACTLConfiguration): QACTL configuration.
    """
    ...
    def __init__(self, provision_info, qa_ctl_configuration):
        ...
        self.fortishield_installation_paths = {}
    ...
    def __process_config_data(self, host_provision_info):
        """Process config file info to generate all the tasks needed for deploy Fortishield
        ...
        if 'fortishield_deployment' in host_provision_info:
            deploy_info = host_provision_info['fortishield_deployment']
            health_check = False if 'health_check' not in host_provision_info['fortishield_deployment'] \
                else host_provision_info['fortishield_deployment']['health_check']
            ...
            fortishield_install_path = '/var/ossec' if 'fortishield_install_path' not in deploy_info else \
                deploy_info['fortishield_install_path']
            fortishield_branch = 'master' if 'fortishield_branch' not in deploy_info else deploy_info['fortishield_branch']
            ...
            installation_files_parameters = {'fortishield_target': install_target}
            ...
            if install_type == 'sources':
                installation_files_parameters['fortishield_branch'] = fortishield_branch
                installation_files_parameters['fortishield_install_path'] = fortishield_install_path
                installation_instance = FortishieldSources(**installation_files_parameters)
            ...
            if install_type == 'package':
                if s3_package_url is None and local_package_path is None:
                    ...
                    installation_instance = FortishieldS3Package(**installation_files_parameters)
                    ...
                    installation_instance = FortishieldLocalPackage(**installation_files_parameters)
                    remote_files_path = installation_instance.download_installation_files(self.inventory_file_path,
                                                                                          hosts=current_host)
                else:
            ...
            if install_target == 'agent':
                deployment_instance = AgentDeployment(remote_files_path,
                                                      ...
                                                      install_dir_path=fortishield_install_path,
            ...
            if health_check:
                # Wait for Fortishield initialization before health_check
                health_check_sleep_time = 30
                QAProvisioning.LOGGER.info(f"Performing a Fortishield installation healthcheck in {current_host} host")
                sleep(health_check_sleep_time)
                deployment_instance.health_check()

            self.fortishield_installation_paths[deployment_instance.hosts] = deployment_instance.install_dir_path

        if 'qa_framework' in host_provision_info:
            qa_framework_info = host_provision_info['qa_framework']
            fortishield_qa_branch = 'master' if 'fortishield_qa_branch' not in qa_framework_info \
                else qa_framework_info['fortishield_qa_branch']

            QAProvisioning.LOGGER.info(f"Provisioning the {current_host} host with the Fortishield QA framework using "
                                       f"{fortishield_qa_branch} branch.")
            qa_instance = QAFramework(qa_branch=fortishield_qa_branch, workdir=qa_framework_info['qa_workdir'],
    ...
    def run(self):
        ...
        if sys.platform == 'win32':
            ...
            tmp_config_file = os.path.join(gettempdir(), 'fortishield_qa_ctl', tmp_config_file_name)
