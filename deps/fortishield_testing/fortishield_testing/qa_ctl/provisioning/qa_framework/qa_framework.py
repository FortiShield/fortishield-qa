...
class QAFramework():
    ...
    def __init__(self, ansible_output=False, workdir=join(gettempdir(), 'fortishield_qa_ctl'), qa_branch='master',
                 qa_repository='https://github.com/fortishield/fortishield-qa.git', ansible_admin_user='vagrant'):
    ...
    def install_dependencies(self, inventory_file_path, hosts='all'):
        ...
        dependencies_unix_task = AnsibleTask({
            ...
            'args': {'chdir': join_path([self.workdir, 'fortishield-qa'],
                                        self.system_path)},
            'become': True,
    ...
    def install_framework(self, inventory_file_path, hosts='all'):
        """Install the fortishield_testing framework to allow the execution of the tests.

        Args:
            inventory_file_path (str): Path were save the ansible inventory.
        """
        install_framework_unix_task = AnsibleTask({
            'name': 'Install fortishield-qa framework (Unix)',
            'shell': 'python3 setup.py install',
            'args': {'chdir': join_path([self.workdir, 'fortishield-qa', 'deps',
                                        'fortishield_testing'], self.system_path)},
            'become': True,
            'when': 'ansible_system != "Win32NT"'
        })

        install_framework_windows_task = AnsibleTask({
            'name': 'Install fortishield-qa framework (Windows)',
        ...
        QAFramework.LOGGER.debug(f"Installing fortishield-qa framework in {hosts} hosts.")

        AnsibleRunner.run_ephemeral_tasks(inventory_file_path, playbook_parameters, output=self.ansible_output)

        QAFramework.LOGGER.debug(f"fortishield-qa framework has been installed successfully in {hosts} hosts.")
    ...
    def download_qa_repository(self, inventory_file_path, hosts='all'):
        ...
        download_qa_repo_unix_task = AnsibleTask({
            'name': f"Download {self.qa_branch} branch of fortishield-qa repository (Unix)",
            'shell': f"cd {self.workdir} && curl -Ls https://github.com/fortishield/fortishield-qa/archive/"
                     f"{self.qa_branch}.tar.gz | tar zx && mv fortishield-* fortishield-qa",
            'when': 'ansible_system != "Win32NT"'
        })

        download_qa_repo_windows_task = AnsibleTask({
            'name': f"Download {self.qa_branch} branch of fortishield-qa repository (Windows)",
            'win_shell': "powershell.exe {{ item }}",
            'with_items': [
                f"curl.exe -L https://github.com/fortishield/fortishield-qa/archive/{self.qa_branch}.tar.gz -o "
                f"{self.workdir}\\{self.qa_branch}.tar.gz",
                f"tar -xzf {self.workdir}\\{self.qa_branch}.tar.gz -C {self.workdir}",
                f"move {self.workdir}\\fortishield-qa-{self.qa_branch} {self.workdir}\\fortishield-qa",
