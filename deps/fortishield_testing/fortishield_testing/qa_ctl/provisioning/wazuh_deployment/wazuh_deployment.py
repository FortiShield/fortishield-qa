...
class FortishieldDeployment(ABC):
    """Deploy Fortishield with all the elements needed, set from the configuration file
    ...
    def install(self, install_type):
        """Installs Fortishield (agent or manager) by creating an ansible playbook and launching it

        Returns:
            AnsibleOutput: Result of the ansible playbook run.
        """
        FortishieldDeployment.LOGGER.debug(f"Installing fortishield {install_type} via {self.install_mode} in {self.hosts} hosts")
        tasks_list = []
        ...
        if self.install_mode == 'sources':
            ...
            tasks_list.append(AnsibleTask({
                'name': 'Executing "install.sh" script to build and install Fortishield',
                'shell': f"./install.sh > {gettempdir()}/fortishield_qa_ctl/fortishield_install_log.txt",
            ...
            tasks_list.append(AnsibleTask({
                'name': 'Install Fortishield Agent from Windows packages',
            ...
            tasks_list.append(AnsibleTask({
                'name': 'Install macOS fortishield package',
        ...
        FortishieldDeployment.LOGGER.debug(f"Fortishield installation has been successfully done in {self.hosts} hosts")

        return tasks_result

    def __control_service(self, command, install_type):
        """Private method to control the Fortishield service in different systems.

        Returns:
            AnsibleOutput: Result of the ansible playbook run.
        """
        FortishieldDeployment.LOGGER.debug(f"{command}ing fortishield service in {self.hosts} hosts")
        tasks_list = []
        service_name = install_type if install_type == 'agent' else 'manager'
        service_command = f'{command}ed' if command != 'stop' else 'stopped'

        tasks_list.append(AnsibleTask({
            'name': f'Fortishield manager {command} service from systemd',
            'become': True,
            'systemd': {'name': f'fortishield-{service_name}', 'state': f'{service_command}'},
        ...
        tasks_list.append(AnsibleTask({
            'name': f'Fortishield agent {command} service from fortishield-control',
            'become': True,
            'command': f'{self.install_dir_path}/bin/fortishield-control {command}',
            'when': 'ansible_system == "Darwin" or ansible_system == "SunOS"'
        }))

        tasks_list.append(AnsibleTask({
            'name': f'Fortishield agent {command} service from Windows',
            'win_shell': 'Get-Service -Name FortishieldSvc -ErrorAction SilentlyContinue |' +
        ...
        tasks_result = AnsibleRunner.run_ephemeral_tasks(self.inventory_file_path, playbook_parameters,
                                                         output=self.qa_ctl_configuration.ansible_output)
        FortishieldDeployment.LOGGER.debug(f"Fortishield service was {command}ed in {self.hosts} hosts")
    ...
    def health_check(self):
        ...
        Returns:
            AnsibleOutput: Result of the ansible playbook run.
        """
        FortishieldDeployment.LOGGER.debug(f"Starting fortishield deployment healthcheck in {self.hosts} hosts")
        tasks_list = []
        tasks_list.append(AnsibleTask({
        ...
        tasks_result = AnsibleRunner.run_ephemeral_tasks(self.inventory_file_path, playbook_parameters,
                                                         output=self.qa_ctl_configuration.ansible_output)
        FortishieldDeployment.LOGGER.debug(f"Fortishield deployment healthcheck OK in {self.hosts} hosts")

        return tasks_result

    def fortishield_is_already_installed(self):
        """Check if Fortishield is installed in the system

        Returns:
            bool: True if fortishield is already installed, False if not
        """
        tasks_list = []
        tasks_list.append(AnsibleTask({
            'name': 'Check Fortishield directory exist',
