...
class AgentDeployment(FortishieldDeployment):
    """Deploy Fortishield agent with all the elements needed, set from the configuration file
    ...
    def install(self):
        """Child method to install Fortishield in agent
    ...
    def register_agent(self):
        """Set the manager ip in the ossec.conf, and restart Fortishield agent to let autoenrollment register it.
    ...
    def health_check(self):
        ...
        tasks_list.append(AnsibleTask({
            ...
            'register': 'status',
            'become': True,
            'failed_when': ['"fortishield-agentd" not in status.stdout',
                            '"fortishield-execd" not in status.stdout']
        }))
