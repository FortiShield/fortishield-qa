...
from fortishield_testing.tools import GLOBAL_DB_PATH, FORTISHIELD_DB_SOCKET_PATH
from fortishield_testing.tools.monitoring import fortishield_pack, fortishield_unpack
from fortishield_testing.tools.services import control_service


def callback_fortishielddb_response(item):
...
def mock_db(func):
    """Decorator used in any function that needs to mock a fortishield db

    This function will execute `func` after stopping fortishield-modulesd and fortishield-db. After that,
    ...
    def magic(*args, **kwargs):
        control_service('stop', daemon='fortishield-modulesd')
        func(*args, **kwargs)
        control_service('start', daemon='fortishield-modulesd')
...
def run_query(db_query, db_path=GLOBAL_DB_PATH):
    """Method used to run sqlite queries on fortishield databases
...
def query_wdb(command):
    ...
    try:
        sock.send(fortishield_pack(len(command)) + command.encode())
