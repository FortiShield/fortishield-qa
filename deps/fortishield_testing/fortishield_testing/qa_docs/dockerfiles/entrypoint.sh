...
git clone https://github.com/fortishield/fortishield-qa --depth=1 -b ${BRANCH} &> /dev/null
...
/usr/local/bin/qa-docs -p /tests/fortishield-qa/tests --validate-parameters ${CMD}
...
echo "Running /usr/local/bin/qa-docs -p /tests/fortishield-qa/tests ${CMD}"
/usr/local/bin/qa-docs -p /tests/fortishield-qa/tests ${CMD}
