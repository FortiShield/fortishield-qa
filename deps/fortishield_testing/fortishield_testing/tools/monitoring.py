...
REMOTED_DETECTOR_PREFIX = r'.*fortishield-remoted.*'
LOG_COLLECTOR_DETECTOR_PREFIX = r'.*fortishield-logcollector.*'
AGENT_DETECTOR_PREFIX = r'.*fortishield-agent.*' if sys.platform == 'win32' else r'.*fortishield-agentd.*'
WINDOWS_AGENT_DETECTOR_PREFIX = r'.*fortishield-agent.*'
AUTHD_DETECTOR_PREFIX = r'.*fortishield-authd.*'
MODULESD_DETECTOR_PREFIX = r'.*fortishield-modulesd.*'
FORTISHIELD_DB_PREFIX = r'.*fortishield-db.*'

DEFAULT_POLL_FILE_TIME = 1
DEFAULT_WAIT_FILE_TIMEOUT = 30


def fortishield_unpack(data, format_: str = "<I"):
    """Unpack data with a given header. Using Fortishield header by default.
...
def fortishield_pack(data, format_: str = "<I"):
    """Pack data with a given header. Using Fortishield header by default.
...
def make_callback(pattern, prefix="fortishield", escape=False):
    """
    ...
    def send(self, message, size=False):
        ...
        Args:
            ...
            size (bool, optional) : Flag that indicates if the header of the message includes the size of the message
                (For example, Analysis doesn't need the size, fortishield-db does). Default `False`
        Returns:
            (int) : Size of the sent message
        """
        msg_bytes = message.encode() if isinstance(message, str) else message
        try:
            msg_bytes = fortishield_pack(len(msg_bytes)) + msg_bytes if size else msg_bytes
...
class StreamHandler(socketserver.BaseRequestHandler):
    ...
    def unix_forward(self, data):
        ...
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as forwarded_sock:
            ...
            forwarded_sock.sendall(fortishield_pack(len(data)) + data)

            # Receive data from the server and shut down
            size = fortishield_unpack(self.recvall_size(forwarded_sock, 4, socket.MSG_WAITALL))
    ...
    def default_fortishield_handler(self):
        """Default fortishield daemons TCP handler method for MITM server."""
        self.request.settimeout(1)
        while not self.server.mitm.event.is_set():
            header = self.recvall_size(self.request, 4, socket.MSG_WAITALL)
            if not header:
                break
            size = fortishield_unpack(header)
            ...
            self.request.sendall(fortishield_pack(len(response)) + response)

    def handle(self):
        """Overriden handle method for TCP MITM server."""
        if self.server.mitm.handler_func is None:
            self.default_fortishield_handler()
        else:
            while not self.server.mitm.event.is_set():
...
class DatagramHandler(socketserver.BaseRequestHandler):
    ...
    def default_fortishield_handler(self):
        """Default fortishield daemons UDP handler method for MITM server."""
        data = self.request[0]
...
class ManInTheMiddle:
    ...
    def run(self, *args):
        ...
        if isinstance(self.listener_socket_address, str):
            uid = pwd.getpwnam('fortishield').pw_uid
