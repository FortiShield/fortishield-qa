...
class ClusterCSVParser:
    """Class to load and parse CSVs with data produced by the Fortishield cluster.
    ...
    def _add_child_process_to_parent_process(files):
        ...
        concat_dfs["Daemon"] = concat_dfs["Daemon"].apply(lambda x: "fortishield-clusterd")
        concat_dfs.reset_index(inplace=True)
        return {'fortishield-clusterd': concat_dfs}
...
class ClusterCSVResourcesParser(ClusterCSVParser):
    """Class to load, parse CSVs and obtain stats of resources used by the 'fortishield-clusterd' process.
    ...
    def __init__(self, artifacts_path, columns=None):
        ...
        super().__init__(artifacts_path, files_to_load=['fortishield-clusterd', 'integrity_sync', 'fortishield-clusterd_child_1',
                                                        'fortishield-clusterd_child_2'])
