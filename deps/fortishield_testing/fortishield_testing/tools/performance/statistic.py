...
import fortishield_testing.tools as tls

logger = logging.getLogger('fortishield-statistics-monitor')
logger.setLevel(logging.INFO)


class StatisticMonitor:
    """This class generates a Python object to monitor the statistics file generated by Fortishield.

    There are four files: fortishield-analysisd.state, fortishield-remoted.state, fortishield-agentd.state and fortishield-logcollector.state
    ...
    def _parse_classic_state_file(self, data):
        """Parse the info from the .state files from Fortishield with shell compatible format.
    ...
    def _parse_state_file(self):
        """Read the data from the statistics file generated by Fortishield."""
        try:
