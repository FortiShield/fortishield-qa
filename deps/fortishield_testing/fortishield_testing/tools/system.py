...
from fortishield_testing.tools import FORTISHIELD_CONF, FORTISHIELD_API_CONF, API_LOG_FILE_PATH, FORTISHIELD_LOCAL_INTERNAL_OPTIONS
...
class HostManager:
    ...
    def move_file(self, host: str, src_path: str, dest_path: str = '/var/ossec/etc/ossec.conf', check: bool = False):
        ...
        if self.get_host_variables(host)['os_name'] == 'windows':
            ...
            result = self.get_host(host).ansible('copy', f'src={src_path} dest={dest_path}'
                                                 'owner=fortishield group=fortishield mode=preserve',
                                                 check=check)
    ...
    def control_service(self, host: str, service: str = 'fortishield', state: str = "started", check: bool = False):
        ...
        Args:
            ...
            check (bool, optional): Ansible check mode("Dry Run"), by default it is enabled so no changes will be
                applied. Default `False`.
        """
        if service == 'fortishield':
            service = 'fortishield-agent' if 'agent' in host else 'fortishield-manager'
    ...
    def apply_config(self, config_yml_path: str, dest_path: str = FORTISHIELD_CONF, clear_files: list = None,
        ...
        for host, payload in config.items():
            ...
            parse_configurations[host] = set_section_fortishield_conf(sections=payload['sections'],
    ...
    def apply_api_config(self, api_config: str or dict = None, host_list: list = None, dest_path: str = FORTISHIELD_API_CONF,
        ...
        for host in host_list:
            self.control_service(host=host, service='fortishield-manager', state='restarted')
            if clear_log:
                self.clear_file(host=host, file_path=API_LOG_FILE_PATH)

    def get_api_token(self, host, user='fortishield', password='fortishield', auth_context=None, port=55000, check=False):
        """Return an API token for the specified user.

        Args:
            host (str): HostName in inventory.
            user (str, optional): API username. Default `fortishield`
            password (str, optional): API password. Default `fortishield`
    ...
    def configure_local_internal_options(self, local_internal_options: dict):
        ...
        for target_host in local_internal_options:
            ...
            backup_local_internal_options = self.get_file_content(target_host, FORTISHIELD_LOCAL_INTERNAL_OPTIONS)
            ...
            self.modify_file_content(target_host, FORTISHIELD_LOCAL_INTERNAL_OPTIONS, replace)
    ...
    def handle_fortishield_services(self, host, operation):
        """
        Handles Fortishield services on the specified host.

        Args:
            host (str): The target host on which to handle Fortishield services.
            operation (str): The operation to perform ('start', 'stop', 'restart').

        Example:
            host_manager.handle_fortishield_services('my_host', 'restart')
        """
        ...
        if os == 'windows':
            if operation == 'restart':
                self.get_host(host).ansible('ansible.windows.win_shell', f'NET stop Fortishield', check=False)
                self.get_host(host).ansible('ansible.windows.win_shell', f'NET start Fortishield', check=False)
            else:
                result = self.get_host(host).ansible('ansible.windows.win_shell', f'NET {operation} Fortishield', check=False)
        else:
            if os == 'linux':
                result = binary_path = f"/var/ossec/bin/fortishield-control"
            elif os == 'macos':
                result= binary_path = f"/Library/Ossec/bin/fortishield-control"
    ...
    def control_environment(self, operation, group_list):
        """
        Controls the Fortishield services on hosts in the specified groups.

        Args:
            operation (str): The operation to perform on Fortishield services ('start', 'stop', 'restart').
            group_list (list): A list of group names whose hosts' Fortishield services should be controlled.
        ...
        for group in group_list:
            for host in self.get_group_hosts(group):
                self.handle_fortishield_services(host, operation)
