#!/usr/bin/python
# Fortishield agents load simulator
...
import fortishield_testing.data.syscollector as syscollector
import fortishield_testing.data.winevt as winevt
import fortishield_testing.fortishield_db as wdb
from fortishield_testing import TCP
from fortishield_testing import is_udp, is_tcp
from fortishield_testing.tools.monitoring import fortishield_unpack, Queue
from fortishield_testing.tools.remoted_sim import Cipher
from fortishield_testing.tools.utils import retry, get_random_ip, get_random_string
...
class Agent:
    ...
    Args:
        ...
        version (str, optional): Fortishield agent version. Default v3.12.0.
        labels (dict, optional): Fortishield agent labels. Each dict key will be a new label.
    ...
    @staticmethod
    def fortishield_padding(compressed_event):
        """Add the Fortishield custom padding to each event sent.
        Args:
            compressed_event (bytes): Compressed event with zlib.
        Returns:
            bytes: Padded event.
        Examples:
            >>> fortishield_padding(b'x\\x9c\\x15\\xc7\\xc9\\r\\x00 \\x08\\x04\\xc0\\x96\\\\\\x94\\xcbn0H\\x03\\xda\\x7f
    ...
    def create_event(self, message):
        ...
        # Padding
        padded_event = self.fortishield_padding(compressed_event)
        # Encrypt
        ...
        while self.stop_receive == 0:
            if is_tcp(sender.protocol):
                try:
                    ...
                    if len(rcv) == 4:
                        data_len = fortishield_unpack(rcv)
    ...
    def process_command(self, sender, message_list):
        ...
        if 'com' in message_list:
            ...
            ['12d95abf04334f90f8dc3140031b3e7b342680000000130:5489:#!-req', '81d15486', 'com', 'close',
                'fortishield_agent_v4.2.0_linux_x86_64.wpk']
            ...
            ['8c0e7a8d75fea76016040ce436f9fb41193290000000130:5491:#!-req', '81d15488', 'com', 'upgrade',
                'fortishield_agent_v4.2.0_linux_x86_64.wpk', 'upgrade.sh']
            """
