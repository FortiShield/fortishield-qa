...
def get_s3_package_url(repository, target, version, revision, system, architecture, install_dir='/var', short_url=True):
    ...
    Args:
        ...
        target (string): Type of the fortishield installation package.
        ...
        install_dir (string): Fortishield installation directory
...
def get_repository_url(target, version, system, revision, repository, architecture, short_url=True):
    ...
    system_path = {
       SYSTEMS['rpm']: '/yum',
       SYSTEMS['deb']: f"/apt/pool/main/w/fortishield-{target}",
...
def get_package_name(target, version, system, revision, repository, architecture):
    ...
    if system == SYSTEMS['rpm']:
        ...
        package_name = f"fortishield_agent_v{version}{revision_section}_linux_x86_64.wpk"
    elif system == SYSTEMS['wpk-windows']:
        revision_section = f"-{revision}" if revision != '1' else ''
        package_name = f"fortishield_agent_v{version}{revision_section}_windows.wpk"
    else:
...
def get_production_package_url(fortishield_target, os_system, fortishield_version):
    ...
    Args:
        ...
        fortishield_version (str): Fortishield version (e.g 4.2.5)
    ...
    return get_s3_package_url('live', fortishield_target, fortishield_version, '1', package_system, architecture)


def get_last_production_package_url(fortishield_target, os_system):
    ...
    """
    last_fortishield_version = github_checks.get_last_fortishield_version()

    return get_production_package_url(fortishield_target, os_system, last_fortishield_version)
