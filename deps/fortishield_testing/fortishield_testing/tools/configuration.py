...
from fortishield_testing import global_parameters, logger, VALID_FIM_MODES, OS_EXCLUDED_FROM_RT_WD
from fortishield_testing.tools import FORTISHIELD_PATH, GEN_OSSEC, FORTISHIELD_CONF, PREFIX, FORTISHIELD_LOCAL_INTERNAL_OPTIONS, AGENT_CONF, \
                                LOCAL_RULES_PATH
...
def set_fortishield_conf(fortishield_conf: List[str]):
    """
    Set up Fortishield configuration. Fortishield will be restarted to apply it.

    Args:
        fortishield_conf (ET.ElementTree): ElementTree with a custom Fortishield configuration.
    """
    write_fortishield_conf(fortishield_conf)
    print("Restarting Fortishield...")
    command = os.path.join(FORTISHIELD_PATH, 'bin/fortishield-control')
    arguments = ['restart']
    check_call([command] + arguments, stdout=DEVNULL, stderr=DEVNULL)


def generate_fortishield_conf(args: List = None) -> ET.ElementTree:
    """
    Generate a configuration file for Fortishield.

    Args:
        args (list, optional): Arguments to generate ossec.conf (install_type, distribuition, version). Default `None`

    Returns:
        (ET.ElementTree): New Fortishield configuration generated from 'gen_ossec.sh'.
    """
    gen_ossec_args = args if args else ['conf', 'manager', 'rhel', '7']
    fortishield_config = check_output([GEN_OSSEC] + gen_ossec_args).decode(encoding='utf-8', errors='ignore')

    return ET.ElementTree(ET.fromstring(fortishield_config))


def get_fortishield_conf() -> List[str]:
    """
    ...
    """
    with open(FORTISHIELD_CONF) as f:
        lines = f.readlines()
    return lines
...
def write_fortishield_conf(fortishield_conf: List[str]):
    """
    Write a new configuration in 'ossec.conf' file.

    Args:
        fortishield_conf (list or str): Lines to be written in the ossec.conf file.
    """
    with open(FORTISHIELD_CONF, 'w') as f:
        f.writelines(fortishield_conf)
...
def write_security_conf(path: str, security_conf: dict):
    ...
    if not os.path.exists(path):
        from fortishield_testing.tools import FORTISHIELD_UID, FORTISHIELD_GID

        open(path, mode='w').close()
        os.chown(uid=FORTISHIELD_UID, gid=FORTISHIELD_GID, path=path)
    write_api_conf(path, security_conf)


def set_section_fortishield_conf(sections, template=None):
    """
    Set a configuration in a section of Fortishield. It replaces the content if it exists.

    Args:
        sections (list): List of dicts with section and new elements
        section (str, optional): Section of Fortishield configuration to replace. Default `'syscheck'`
    ...
    Returns:
        List of str: List of str with the custom Fortishield configuration.
    """

    def create_elements(section: ET.Element, elements: List):
        """
        Insert new elements in a Fortishield configuration section.

        Args:
            section (ET.Element): Section where the element will be inserted.
            elements (list): List with the new elements to be inserted.
        Returns:
            ET.ElementTree: Modified Fortishield configuration.
        """
        tag = None
    ...
    def find_module_config(fortishield_conf: ET.ElementTree, section: str, attributes: List[dict]) -> ET.ElementTree:
        r"""
        ...
        Args:
            fortishield_conf (ElementTree): An ElementTree object with all the data of the ossec.conf
        ...
        if attributes is None:
            return fortishield_conf.find(section)
        else:
            ...
            try:
                return fortishield_conf.find(query)
            except AttributeError:
                return None

    # Get Fortishield configuration as a list of str
    raw_fortishield_conf = get_fortishield_conf() if template is None else template
    # Generate a ElementTree representation of the previous list to work with its sections
    fortishield_conf = to_elementTree(purge_multiple_root_elements(raw_fortishield_conf))
    for section in sections:
        attributes = section.get('attributes')
        section_conf = find_module_config(fortishield_conf, section['section'], attributes)
        # Create section if it does not exist, clean otherwise
        if not section_conf:
            section_conf = ET.SubElement(fortishield_conf.getroot(), section['section'])
            section_conf.text = '\n    '
            section_conf.tail = '\n\n  '
        else:
    ...
    return to_str_list(fortishield_conf)
...
def load_fortishield_configurations(yaml_file_path: str, test_name: str, params: list = None, metadata: list = None) -> Any:
    r"""
    Load different configurations of Fortishield from a YAML file.
...
def get_fortishield_local_internal_options() -> List[str]:
    """Get current `internal_options.conf` file content.

    Returns
        List of str: A list containing all the lines of the `ossec.conf` file.
    """
    with open(FORTISHIELD_LOCAL_INTERNAL_OPTIONS) as f:
        lines = f.readlines()
    return lines


def set_fortishield_local_internal_options(fortishield_local_internal_options: List[str]):
    """Set up Fortishield `local_internal_options.conf` file content.

    Returns
        List of str: A list containing all the lines of the `local_interal_options.conf` file.
    """
    with open(FORTISHIELD_LOCAL_INTERNAL_OPTIONS, 'w') as f:
        f.writelines(fortishield_local_internal_options)


def add_fortishield_local_internal_options(fortishield_local_internal_options_dict):
    """Add new local internal options to the current configuration.

    Args:
        fortishield_local_internal_options_dict (List of str): A list containing local internal options to add.
    """
    local_internal_options_str = create_local_internal_options(fortishield_local_internal_options_dict)
    with open(FORTISHIELD_LOCAL_INTERNAL_OPTIONS, 'a') as f:
...
def create_local_internal_options(dict_local_internal_options):
    ...
    for key, value in dict_local_internal_options.items():
        fortishield_local_internal_options += f"{str(key)}={str(value)}\n"
    return fortishield_local_internal_options
...
def get_local_internal_options_dict():
    ...
    with open(FORTISHIELD_LOCAL_INTERNAL_OPTIONS, 'r') as local_internal_option_file:
...
def set_local_internal_options_dict(dict_local_internal_options):
    ...
    """
    with open(FORTISHIELD_LOCAL_INTERNAL_OPTIONS, 'w') as local_internal_option_file:
...
def load_configuration_template(data_file_path, configuration_parameters=[], configuration_metadata=[]):
    ...
    Returns:
        list(dict): List containing fortishield configurations in dictionary form.
...
def get_fortishield_local_rules():
    """
...
def write_fortishield_local_rules(local_rules: List[str]):
    """
...
def get_minimal_configuration():
    """Get the fortishield minimal configuration data.

    Returns:
        str: Fortishield minimal configuration data.
    """
