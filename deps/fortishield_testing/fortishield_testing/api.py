...
def compare_config_api_response(configuration, section):
    ...
    Args:
        configuration (dict): Dictionary with Fortishield manager configuration.
...
def get_manager_configuration(section=None, field=None):
    ...
    Args:
        section (str): fortishield configuration section, E.g: "active-response", "ruleset"...
        field   (str): section child. E.g, fields for ruleset section are: decoder_dir, rule_dir, etc

    Returns:
        `obj`(str or map): active configuration indicated by Fortishield API. If section and field are selected, it will
    ...
    assert response.json()['error'] == 0, f"Fortishield API response status different from 0: {response.json()}"
...
def wait_until_api_ready(protocol=API_PROTOCOL, host=API_HOST, port=API_PORT, user=API_USER, password=API_PASS,
    ...
    Args:
        protocol (str): Used protocol for Fortishield manager.
        host (str): Fortishield manager host ip.
...
def make_api_call(manager_address=API_HOST, port=55000, method='GET', endpoint='/', headers=None, request_json=None,
    ...
    Args:
        port (str, optional): Fortishield manager port.
