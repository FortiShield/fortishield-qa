...
from fortishield_testing.tools import LOGCOLLECTOR_STATISTICS_FILE, FORTISHIELD_PATH, monitoring
...
DEFAULT_AUTHD_REMOTED_SIMULATOR_CONFIGURATION = {
    ...
    'server_keys': os.path.join(FORTISHIELD_PATH, 'etc', 'sslmanager.key'),
    'server_cert': os.path.join(FORTISHIELD_PATH, 'etc', 'sslmanager.cert'),
    'authd_port': 1515,
    'remoted_port': 1514,
    'protocol': 'tcp',
...
def callback_unable_to_open(file_path, n_attempt):
    """Create a callback to detect if `fortishield-logcollector` fails to open the specified file.
    Args:
        file_path (str): Path of the file that `fortishield-logcollector` is trying to open.
...
def callback_trying_to_reconnect(location, reconnect_time):
    """Create a callback to detect if `fortishield-agentd` is trying to reconnect to specified channel.
    Args:
...
def callback_invalid_format_value(line, option, location):
    ...
    Args:
        ...
        location (str): Fortishield manager configuration option.
...
def get_data_sending_stats(log_path, socket_name):
    ...
    For this purpose, it parses the "fortishield-logcollector.state" file and retrieves the data.
    See:
    https://documentation-dev.fortishield.github.io/current/user-manual/reference/statistics-files/fortishield-logcollector-state.html
...
def get_next_stats(current_stats, log_path, socket_name, state_interval):
    """Return the next statistics to be written to the "fortishield-logcollector.state" file and the seconds elapsed.
...
def compose_macos_log_command(type='', level='', predicate='', is_sierra=False):
    """
    This function replicates how the command 'log' will be called from the Fortishield agent given the query parameters
