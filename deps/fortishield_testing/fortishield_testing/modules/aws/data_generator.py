...
from fortishield_testing.tools.utils import get_random_ip, get_random_port, get_random_string
        ...
        return json.dumps(
            {
                ...
                'detail': {
                    ...
                    'actor': 'resources.fortishield.github.io',
                    'summary': {
                        'Description': 'S3 Bucket uses IAM policy to grant read rights to Everyone.',
                        'Bucket': {
                            'resources.fortishield.github.io': 1
                        },
                        'Record Count': 1,
                        'ACL': {
                            'resources.fortishield.github.io': [
                                {
                                    'Owner': {
                                        'DisplayName': 'fortishield',
...
class ServerAccessDataGenerator(DataGenerator):
    ...
    def get_data_sample(self):
        ...
        for _ in range(5):
            data.append(
                [
                    str(uuid4()), 'fortishield-server-access-integration-tests',
                    ...
                    'REST.GET.WEBSITE', '-', 'GET, /fortishield-server-access-integration-tests?website= HTTP/1.1',
