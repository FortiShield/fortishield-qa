...
from fortishield_testing.qa_ctl.deployment.qa_infraestructure import QAInfraestructure
from fortishield_testing.qa_ctl.provisioning.qa_provisioning import QAProvisioning
from fortishield_testing.qa_ctl.run_tests.qa_test_runner import QATestRunner
from fortishield_testing.qa_ctl.run_tasks.qa_tasks_launcher import QATasksLauncher
from fortishield_testing.qa_ctl.configuration.qa_ctl_configuration import QACTLConfiguration
...
from fortishield_testing.tools.file import recursive_directory_creation
...
TASKS_KEY = 'tasks'
TEST_KEY = 'tests'
FORTISHIELD_QA_FILES = os.path.join(gettempdir(), 'fortishield_qa_ctl', 'fortishield-qa')
...
def validate_configuration_data(configuration_data, qa_ctl_mode):
    ...
    if sys.platform == 'win32' and qa_ctl_mode == MANUAL_MODE:
        ...
        if not github_checks.branch_exists(configuration_data['config']['qa_ctl_launcher_branch'],
                                           repository=FORTISHIELD_QA_REPO):
            raise QAValueError(f"{configuration_data['config']['qa_ctl_launcher_branch']} branch specified as "
                               'qa_ctl_launcher_branch  does not exist in Fortishield QA repository.', qactl_logger.error,
                               QACTL_LOGGER)
...
def validate_test_module(type=None, component=None, suite=None, module=None):
    ...
    Args:
        ...
        module (str): Test module.
    """
    tests_path = os.path.join(FORTISHIELD_QA_FILES, 'tests')
...
def set_parameters(parameters):
    ...
    try:
        parameters.version = parameters.version if parameters.version else github_checks.get_last_fortishield_version()
    except QAValueError:
        raise QAValueError('The latest version of Fortishield could not be obtained. Maybe there is no valid (non-rc) one at '
                           'https://github.com/fortishield/fortishield/tags. Try specifying the version manually using the '
...
def set_environment(parameters):
    ...
    Args:
        (argparse.Namespace): Object with the user parameters.
    """
    # Create the fortishield_qa_ctl temporary folder
    recursive_directory_creation(os.path.join(gettempdir(), 'fortishield_qa_ctl'))

    if parameters.run:
        # Download fortishield-qa repository locally to run qa-docs tool and get the tests info
        local_actions.download_local_fortishield_qa_repository(branch=parameters.qa_branch,
                                                         path=os.path.join(gettempdir(), 'fortishield_qa_ctl'))
...
def validate_parameters(parameters):
    ...
    Raises:
        QAValueError: If parameters are incompatible, or version has not a valid format, or the specified fortishield version
                      has not been released, or fortishield QA branch does not exist (calculated from fortishield_version).
    """
    ...
    if not github_checks.version_is_released(parameters.version):
        raise QAValueError(f"The fortishield {parameters.version} version has not been released. Enter a right version.",
                           qactl_logger.error, QACTL_LOGGER)
    ...
    # Check if specified tests exist. Fortishield-qa repository needs to be downloaded locally before.
    if parameters.run:
...
def get_script_parameters():
    ...
    description = \
        ...
        Description: qa-ctl is a tool for launching tests locally, automating the deployment, provisioning and testing
                     ...
                     You can find more information in https://github.com/fortishield/fortishield-qa/wiki/QACTL-tool
        '''
    ...
    parser.add_argument('--qa-branch', type=str, action='store', required=False, dest='qa_branch',
                        help='Set a custom fortishield-qa branch to use in the run and provisioning. This '
...
def main():
    ...
    if qa_ctl_mode == AUTOMATIC_MODE:
        ...
        config_generator = QACTLConfigGenerator(modules_data, arguments.version, arguments.qa_branch,
                                                FORTISHIELD_QA_FILES, arguments.operating_systems)
